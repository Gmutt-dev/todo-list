...
// It's been a long time since I have written a vanilla js app so I'm not sure
// I can comment on whether this is a good apprach or not.
...
function deregisterUserHandler(event) {
  ...
  else emitter.emit("fail:userDeregistered", {});
}
...
function logOutUserHandler() {
  if (clearUserSession())
    emitter.emit("userSessionUpdated", getUserSessionClone());
  else emitter.emit("fail:userLoggedOut", {});
}
...
function addProjectHandler(event) {
  if (addProject(getLoggedInUser(), event.projectName))
    emitter.emit("userSessionUpdated", getUserSessionClone());
  else emitter.emit("fail:addProject", {});
}
...
function addTaskHandler(event) {
  if (
    ...
    emitter.emit("userSessionUpdated", getUserSessionClone());
  else emitter.emit("fail:addTask", {});
}
