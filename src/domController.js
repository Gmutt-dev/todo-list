...
function drawDefaultApp() {
  ...
  function drawDefaultMainSectionContents() {
    ...
    mainSection.appendChild(
      createHTMLElement("p", {
        textContent:
          "Please log in or register a new user (see top right of page)",
      }),
    );
  }
  ...
  function toHTMLElement(project) {
      ...
      if (!e.target.parentElement.classList.contains("user-selected")) {
        ...
        projectsList.querySelectorAll(".project").forEach((element) => {
          ...
          element
            ...
            .forEach((button) => button.classList.add("not-displayed"));
          element
            .querySelectorAll("button:not(:first-child")
            .forEach((button) => button.classList.add("not-displayed"));
        });
    ...
    editButton.addEventListener("click", (e) => {
      ...
      projectsList
        ...
        .forEach((element) => element.classList.remove("user-selected"));
      projectButton.parentElement.classList.add("user-selected");
      drawTasksSection(userSession);
    ...
    deleteButton.addEventListener("click", (e) => {
      ...
      emitter.emit("request:deleteProject", {
        projectId: e.target.parentElement.dataset.id,
      });
    });
...
function drawTasksHeader() {
  ...
  if (userSelection.selectedProjectId) p.textContent = "Tasks:";
    ...
    p.textContent =
      "Please select a project from the left bar, to show related tasks";
  }
...
function drawUserTasks(userSession) {
  ...
  userSession.userTasks
    .filter((task) =>
      userSelection.selectedProjectId
        ? task.linkedProjectId === userSelection.selectedProjectId
        : false,
    )
    .map(toHTMLElement)
  ...
  function toHTMLElement(task) {
    ...
    taskCard.toEditable = function () {
      ...
      submitButton.addEventListener("click", (e) => {
        ...
        emitter.emit("request:updateTask", {
          ...
          description: taskCard.querySelector(`input[name="description"]`)
            .value,
          dueDate: new Date(
            taskCard.querySelector(`input[name="dueDate"]`).value,
          ),
      ...
      cancelButton.addEventListener("click", (e) => {
        ...
        if (taskCard.classList.contains("new-task"))
    ...
    taskCard.toCompact = function () {
      taskCard.querySelectorAll(".displayable").forEach((item) => {
        if (item.classList.contains("displayed"))
          item.classList.replace("displayed", "not-displayed");
      });
    };
    ...
    taskCard.querySelector(".compact-button").addEventListener("click", (e) => {
      ...
      userSelection.selectedTaskIds.splice(
        userSelection.selectedTaskIds.findIndex((id) => id === task.id),
        1,
      );
    });
    ...
    taskCard.querySelectorAll("output").forEach((output) => {
      if (output.name === "dueDate")
        output.value = format(task.dueDate, "dd-MMM-yyyy");
      else if (output.name === "isDone")
        output.value = task.isDone ? "yes" : "no";
    ...
    taskCard.expand = function () {
      taskCard.querySelectorAll(".displayable").forEach((item) => {
        if (item.classList.contains("not-displayed"))
          item.classList.replace("not-displayed", "displayed");
      });
    };
    ...
    if (
      taskCard.querySelector("output[name='title']").value ===
      "<New Task, please complete>"
    ) {
    ...
    taskCard.addEventListener("click", (e) => {
      if (
        !userSelection.selectedTaskIds.includes(task.id) &&
        e.target !== taskCard.querySelector(".compact-button")
      ) {
        taskCard.expand();
